...
from pysceptre.glm.irls import fit_poisson_glm_batch, fit_binomial_glm_batch
...
def test_theta_matches_r_per_gene(ground_truth):
    ...
    for gene in ground_truth["genes"]:
        y = np.array(gene["y"], dtype=float)
        fit = fit_poisson_glm_batch(X, y)
        theta_est, _method = estimate_theta(y=y, mu=fit.fitted_values, dfr=n - p, limit=50, eps=np.finfo(float).eps ** 0.25)
...
def test_binomial_fitted_probabilities_match_r_per_target(ground_truth):
    ...
    for target in ground_truth["targets"]:
        ...
        np.testing.assert_allclose(fit.fitted_values, target["fitted_probabilities"], rtol=1e-6, atol=1e-8)
...
def test_standalone_nb_theta(ground_truth):
    ...
    y = np.array(st["y"])
    mu = np.array(st["mu"])
    theta_est, method = estimate_theta(y=y, mu=mu, dfr=st["dfr"], limit=50, eps=np.finfo(float).eps ** 0.25)
