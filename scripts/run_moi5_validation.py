...
def main():
    ...
    merged["neglog10_p_py"] = -np.log10(
        np.clip(merged["p_value_py"] if "p_value_py" in merged else merged["p_value"], 1e-300, None)
    )
    ...
    mean_abs_log2fc_diff = np.mean(np.abs(np.log2(merged[fc_py_col]) - np.log2(merged[fc_r_col])))
    ...
    py_strong = set(
        zip(
            merged.loc[merged[p_py_col] < 1e-4, "response_id"],
            merged.loc[merged[p_py_col] < 1e-4, "grna_target"],
            strict=True,
        )
    )
    r_strong = set(
        zip(
            merged.loc[merged[p_r_col] < 1e-4, "response_id"],
            merged.loc[merged[p_r_col] < 1e-4, "grna_target"],
            strict=True,
        )
    )
    jaccard = (
        len(py_strong & r_strong) / len(py_strong | r_strong)
        if (py_strong | r_strong)
        else float("nan")
    )
    ...
    print(
        f"p<1e-4 calls: pysceptre={len(py_strong)}  R={len(r_strong)}  "
        f"both={len(py_strong & r_strong)}  Jaccard={jaccard:.4f}"
    )

    merged["abs_neglog10p_diff"] = np.abs(
        -np.log10(np.clip(merged[p_py_col], 1e-300, None))
        - -np.log10(np.clip(merged[p_r_col], 1e-300, None))
    )
    print("\nTop 10 biggest -log10(p) disagreements:")
    print(
        merged.sort_values("abs_neglog10p_diff", ascending=False)[
            ["response_id", "grna_target", p_py_col, p_r_col, fc_py_col, fc_r_col]
        ]
        .head(10)
